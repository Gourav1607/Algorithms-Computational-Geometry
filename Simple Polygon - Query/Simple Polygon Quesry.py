...
def drawpoly(N):
    ...
    for i, vtx in enumerate(scords):
        if i<len(scords)-1:
            cvimage = cv2.line(cvimage, (scords[i][0], scords[i][1]), (scords[i+1][0], scords[i+1][1]), color=(0, 0, 0), thickness=1)
        else:
            cvimage = cv2.line(cvimage, (scords[i][0], scords[i][1]), (scords[0][0], scords[0][1]), color=(0, 0, 0), thickness=1)
...
def shootray(images, cvimage, scords):
    ...
    cvimage = cv2.line(cvimage, (Q[0], Q[1]), (Q[0], Q[1]), color=(0, 0, 255), thickness=1)
    ...
    for i in range(Q[0], SIZE-1, 1):
        ...
        if imvalue[0] == 0 and imvalue[1] == 0 and imvalue[2] == 0:
            totalcross += 1
            i += 1
        cvimage = cv2.line(cvimage, (point[0], point[1]), (point[0], point[1]), color=(0, 0, 255), thickness=1)
    ...
    cv2.putText(cvimage, text, tuple([Q[0], Q[1]-5]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, 1)
    ...
    if totalcross%2!=0:
        vert0 = scords[0]
        vert1 = scords[1]
        cvimage = cv2.line(cvimage, (Q[0], Q[1]), vert0, color=(0, 255, 0), thickness=2)
        cvimage = cv2.line(cvimage, (Q[0], Q[1]), vert1, color=(0, 255, 0), thickness=2)
        cvimage = cv2.line(cvimage, vert1, vert0, color=(0, 255, 0), thickness=2)

        seedpoint = tuple(np.mean([list(Q), list(vert0), list(vert1)], axis=0, dtype='int32'))
        print(seedpoint)
        cv2.floodFill(cvimage, None, seedPoint=seedpoint, newVal=(0, 255, 0), loDiff=(5, 5, 5, 5), upDiff=(5, 5, 5, 5))
...
for N in range(5, 26, 5):
    ...
    if len(images) == 5:
        ...
        ax[3].imshow(cv2.cvtColor(np.zeros((800, 800, 3), dtype='uint8'), cv2.COLOR_BGR2RGB))
        ax[3].set_title('NA')
    plt.tight_layout()
    plt.savefig('CP02_{:02d}.png'.format(N), dpi=300, bbox_inches='tight', pad_inches=0.1)
    plt.show()
