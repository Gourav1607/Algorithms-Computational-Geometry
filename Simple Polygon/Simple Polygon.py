...
def drawncolorpoly(N):
    ...
    for i, vtx in enumerate(scords):
        if i<len(scords)-1:
            cvimage = cv2.line(cvimage, (scords[i][0], scords[i][1]), (scords[i+1][0], scords[i+1][1]), color=(0, 0, 0), thickness=2)
        else:
            cvimage = cv2.line(cvimage, (scords[i][0], scords[i][1]), (scords[0][0], scords[0][1]), color=(0, 0, 0), thickness=2)

    cv2.imwrite('CP01_{:02d}_01.png'.format(N), cvimage)
    images.append(np.array(cvimage))

    cv2.floodFill(cvimage, None, seedPoint=(refX, refY), newVal=(0, 255, 0), loDiff=(5, 5, 5, 5), upDiff=(5, 5, 5, 5))
    cv2.floodFill(cvimage, None, seedPoint=(0, 0), newVal=(0, 0, 255), loDiff=(5, 5, 5, 5), upDiff=(5, 5, 5, 5))
    ...
    plt.tight_layout()
    plt.savefig('CP01_{:02d}.png'.format(N), dpi=300, bbox_inches='tight', pad_inches=0.1)
    plt.show()
